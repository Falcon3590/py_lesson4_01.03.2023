...
# n, m = map(int, input('Введите кол-во элементов первого множеств n и m  через пробел: ').split())
# a = set(map(int, input('Введите элементы множества a ').split()))
# b = set(map(int, input('Введите элементы множества b ').split()))
# print(*sorted(a & b))
...
# x = list(map(int, input('Введите количество ягод на каждом кусте: ').split()))
# max = 0
# for i in range(n):
# 	temp = sum(x[i:i+3])
# 	if temp > max:
